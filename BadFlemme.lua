...
-- =============================================
-- AUTO PIANO
-- Format sheet : "C4 D4 E4 F4:0.5 G4 A4 B4"
-- Note:durée (durée optionnelle en secondes)
-- Séparateur : espace ou nouvelle ligne
-- Notes supportées : C D E F G A B + octave 1-8
-- Dièses : C# D# F# G# A#
-- =============================================

local pianoRunning = false
local pianoThread = nil

-- Map note → KeyCode (touches standard piano Roblox)
local NOTE_KEYS = {
    -- Octave basse (touches bas)
    ["C3"]  = Enum.KeyCode.Z,
    ["D3"]  = Enum.KeyCode.X,
    ["E3"]  = Enum.KeyCode.C,
    ["F3"]  = Enum.KeyCode.V,
    ...
    ["G#4"] = Enum.KeyCode.O,
    ["A4"]  = Enum.KeyCode.P,
    ["A#4"] = Enum.KeyCode.LeftBracket,
    ["B4"]  = Enum.KeyCode.RightBracket,
    -- Octave haute
    ["C5"]  = Enum.KeyCode.A,
    ["C#5"] = Enum.KeyCode.S,
    ["D5"]  = Enum.KeyCode.D,
    ...
    ["G5"]  = Enum.KeyCode.K,
    ["G#5"] = Enum.KeyCode.L,
    ["A5"]  = Enum.KeyCode.Semicolon,
    ["A#5"] = Enum.KeyCode.Quote,
...
-- Parse une sheet en liste de {note, duration}
local function parseSheet(sheet)
    local notes = {}
    for token in sheet:gmatch("%S+") do
        local note, dur = token:match("^([A-Gb#]+%d*)(:?)([%d%.]*)$")
        if note then
            local duration = (dur == ":" and tonumber(token:match(":([%d%.]+)$"))) or 0.15
            table.insert(notes, {note = note:upper(), duration = duration})
        end
    end
    return notes
end

local function playSheet(sheet, statusLbl)
    local notes = parseSheet(sheet)
    if #notes == 0 then
        if statusLbl then statusLbl.Text = "❌ Aucune note trouvée !" end
        return
    end
    pianoRunning = true
    pianoThread = task.spawn(function()
        for i, entry in ipairs(notes) do
            if not pianoRunning then break end
            local keyCode = NOTE_KEYS[entry.note]
            if statusLbl then
                statusLbl.Text = "▶ " .. entry.note .. "  (" .. i .. "/" .. #notes .. ")"
            end
            if keyCode then
                pressKey(keyCode)
            end
        ...
        pianoRunning = false
        if statusLbl then statusLbl.Text = "✅ Terminé !" end
    end)
end

local function stopPiano(statusLbl)
    pianoRunning = false
    if statusLbl then statusLbl.Text = "⏹ Arrêté" end
end
...
local tabButtons = {}
local tabs = {"AFK", "MAIN", "ESP", "FPS", "AIM", "MISC", "PLAYER", "PIANO"}
...
-- Row 2 : AIM MISC PLAYER PIANO (4 tabs)
local row1 = {"AFK", "MAIN", "ESP", "FPS"}
local row2 = {"AIM", "MISC", "PLAYER", "PIANO"}
...
-- PIANO TAB
-- =============================================
local pianoContent = contentContainers["PIANO"]

-- Titre
local pianoTitle = Instance.new("TextLabel")
pianoTitle.Size = UDim2.new(1, -8, 0, 20)
pianoTitle.Position = UDim2.new(0, 4, 0, 4)
pianoTitle.BackgroundTransparency = 1
pianoTitle.Text = "🎹 Auto Piano"
pianoTitle.Font = Enum.Font.GothamBold
pianoTitle.TextSize = 12
pianoTitle.TextColor3 = COLORS.White
pianoTitle.TextXAlignment = Enum.TextXAlignment.Left
pianoTitle.Parent = pianoContent

-- Info format
local pianoInfo = Instance.new("Frame")
pianoInfo.Size = UDim2.new(1, -8, 0, 44)
pianoInfo.Position = UDim2.new(0, 4, 0, 26)
pianoInfo.BackgroundColor3 = COLORS.Frame
pianoInfo.BorderSizePixel = 0
pianoInfo.Parent = pianoContent
createCorner(pianoInfo, 8)
createStroke(pianoInfo, COLORS.Primary, 1)
local pianoInfoLbl = Instance.new("TextLabel")
pianoInfoLbl.Size = UDim2.new(1, -12, 1, 0)
pianoInfoLbl.Position = UDim2.new(0, 6, 0, 0)
pianoInfoLbl.BackgroundTransparency = 1
pianoInfoLbl.Text = "Format : C4 D4 E4:0.3 F4 G4\nNote:durée(s)  •  C D E F G A B + # + octave"
pianoInfoLbl.Font = Enum.Font.Gotham
pianoInfoLbl.TextSize = 9
pianoInfoLbl.TextColor3 = Color3.fromRGB(180, 180, 180)
pianoInfoLbl.TextXAlignment = Enum.TextXAlignment.Left
pianoInfoLbl.TextWrapped = true
pianoInfoLbl.Parent = pianoInfo

-- Zone de saisie sheet (grande TextBox scrollable)
local sheetBg = Instance.new("Frame")
sheetBg.Size = UDim2.new(1, -8, 0, 130)
sheetBg.Position = UDim2.new(0, 4, 0, 76)
sheetBg.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
sheetBg.BorderSizePixel = 0
sheetBg.Parent = pianoContent
createCorner(sheetBg, 8)
createStroke(sheetBg, COLORS.Primary, 1)

local sheetLabel = Instance.new("TextLabel")
sheetLabel.Size = UDim2.new(1, -8, 0, 16)
sheetLabel.Position = UDim2.new(0, 6, 0, 4)
sheetLabel.BackgroundTransparency = 1
sheetLabel.Text = "Sheet :"
sheetLabel.Font = Enum.Font.GothamSemibold
sheetLabel.TextSize = 9
sheetLabel.TextColor3 = Color3.fromRGB(140, 140, 140)
sheetLabel.TextXAlignment = Enum.TextXAlignment.Left
sheetLabel.Parent = sheetBg

local sheetBox = Instance.new("TextBox")
sheetBox.Size = UDim2.new(1, -12, 0, 104)
sheetBox.Position = UDim2.new(0, 6, 0, 20)
sheetBox.BackgroundTransparency = 1
sheetBox.Text = ""
sheetBox.PlaceholderText = "Ex: C4 D4 E4 F4 G4 A4 B4 C5\nColle ta sheet ici..."
sheetBox.PlaceholderColor3 = Color3.fromRGB(80, 80, 80)
sheetBox.Font = Enum.Font.Gotham
sheetBox.TextSize = 10
sheetBox.TextColor3 = COLORS.White
...
pianoStatusBg.Size = UDim2.new(1, -8, 0, 24)
pianoStatusBg.Position = UDim2.new(0, 4, 0, 212)
pianoStatusBg.BackgroundColor3 = COLORS.Frame
pianoStatusBg.BorderSizePixel = 0
pianoStatusBg.Parent = pianoContent
createCorner(pianoStatusBg, 6)

local pianoStatusLbl = Instance.new("TextLabel")
pianoStatusLbl.Size = UDim2.new(1, -8, 1, 0)
pianoStatusLbl.Position = UDim2.new(0, 6, 0, 0)
pianoStatusLbl.BackgroundTransparency = 1
pianoStatusLbl.Text = "⏸ En attente d'une sheet..."
pianoStatusLbl.Font = Enum.Font.GothamSemibold
pianoStatusLbl.TextSize = 10
pianoStatusLbl.TextColor3 = Color3.fromRGB(160, 160, 160)
pianoStatusLbl.TextXAlignment = Enum.TextXAlignment.Left
pianoStatusLbl.Parent = pianoStatusBg

-- Bouton PLAY
local playBtn = Instance.new("TextButton")
playBtn.Size = UDim2.new(0.48, -4, 0, 30)
playBtn.Position = UDim2.new(0, 4, 0, 242)
...
playBtn.Parent = pianoContent
createCorner(playBtn, 8)

-- Bouton STOP
local stopBtn = Instance.new("TextButton")
stopBtn.Size = UDim2.new(0.48, -4, 0, 30)
stopBtn.Position = UDim2.new(0.52, 0, 0, 242)
...
stopBtn.Parent = pianoContent
createCorner(stopBtn, 8)

-- Slider vitesse
local speedSliderLbl = Instance.new("TextLabel")
speedSliderLbl.Size = UDim2.new(1, -8, 0, 16)
speedSliderLbl.Position = UDim2.new(0, 4, 0, 278)
speedSliderLbl.BackgroundTransparency = 1
speedSliderLbl.Text = "⚡ Vitesse : x1.0"
speedSliderLbl.Font = Enum.Font.GothamSemibold
speedSliderLbl.TextSize = 10
speedSliderLbl.TextColor3 = COLORS.White
speedSliderLbl.TextXAlignment = Enum.TextXAlignment.Left
speedSliderLbl.Parent = pianoContent

local speedBg = Instance.new("Frame")
speedBg.Size = UDim2.new(1, -8, 0, 10)
speedBg.Position = UDim2.new(0, 4, 0, 298)
speedBg.BackgroundColor3 = COLORS.Gray
speedBg.BorderSizePixel = 0
speedBg.Parent = pianoContent
createCorner(speedBg, 5)

local speedFill = Instance.new("Frame")
speedFill.Size = UDim2.new(0.5, 0, 1, 0)
speedFill.BackgroundColor3 = COLORS.Primary
speedFill.BorderSizePixel = 0
speedFill.Parent = speedBg
createCorner(speedFill, 5)

local speedKnob = Instance.new("Frame")
speedKnob.Size = UDim2.new(0, 16, 0, 16)
speedKnob.Position = UDim2.new(0.5, -8, 0.5, -8)
speedKnob.BackgroundColor3 = COLORS.White
speedKnob.BorderSizePixel = 0
speedKnob.Parent = speedBg
createCorner(speedKnob, 8)
createStroke(speedKnob, COLORS.Primary, 2)

-- Multiplicateur de vitesse : 0.5x à 3x
local pianoSpeedMult = 1.0
local speedDragging = false
local function updateSpeedSlider(input)
    local pos = math.clamp((input.Position.X - speedBg.AbsolutePosition.X) / speedBg.AbsoluteSize.X, 0, 1)
    pianoSpeedMult = 0.5 + pos * 2.5 -- 0.5 à 3.0
    speedSliderLbl.Text = "⚡ Vitesse : x" .. string.format("%.1f", pianoSpeedMult)
    speedFill.Size = UDim2.new(pos, 0, 1, 0)
    speedKnob.Position = UDim2.new(pos, -8, 0.5, -8)
end
speedBg.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then speedDragging = true updateSpeedSlider(i) end end)
speedBg.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then speedDragging = false end end)
UserInputService.InputChanged:Connect(function(i)
    if speedDragging and i.UserInputType == Enum.UserInputType.MouseMovement then updateSpeedSlider(i) end
end)

-- Actions boutons
playBtn.MouseButton1Click:Connect(function()
    if pianoRunning then
        stopPiano(pianoStatusLbl)
        task.wait(0.1)
    end
    local sheet = sheetBox.Text
    if sheet == "" then
        pianoStatusLbl.Text = "❌ Entre une sheet d'abord !"
        pianoStatusLbl.TextColor3 = COLORS.Red
        return
    end
    pianoStatusLbl.TextColor3 = COLORS.Green
    -- Applique le multiplicateur de vitesse à chaque note
    local speedSheet = sheet:gsub(":([%d%.]+)", function(dur)
        return ":" .. string.format("%.2f", tonumber(dur) / pianoSpeedMult)
    end)
    -- Si pas de durée spécifiée, ajoute la durée de base divisée par la vitesse
    local baseDur = string.format("%.2f", 0.15 / pianoSpeedMult)
    local processedSheet = speedSheet:gsub("(%S+)", function(token)
        if not token:find(":") then
            return token .. ":" .. baseDur
        end
        return token
    end)
    playSheet(processedSheet, pianoStatusLbl)
    notifyImportant("🎹 Piano démarré !")
end)
